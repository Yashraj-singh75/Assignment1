...
{ 
  ...
  { 
    return res.status(err.status || 500).json({ message: err.message || "cannot update product" });
  }
};   
...
{   
  ...
  { 
    return res.status(err.status || 500).json({ message: err.message || "cannot delete product" });
  }
};   
